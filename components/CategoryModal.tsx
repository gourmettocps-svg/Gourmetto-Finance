...
const CategoryModal: React.FC<CategoryModalProps> = ({ isOpen, onClose, onSave, initialName = '' }) => {
      ...
      <div className="bg-white w-full max-w-sm rounded-sm shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Nome da Categoria</label>
            <input 
              required
              autoFocus
              type="text"
          ...
          <div className="flex gap-2">
            ...
            >
              {initialName ? 'Atualizar' : 'Adicionar'}
            </button>
          </div>
        </form>
      </div>
